...
export default function AdminPayments() {
  ...
  const fetchData = async () => {
    ...
    try {
      // جلب المدفوعات
      const payRes = await fetch("http://127.0.0.1:8005/api/admin/payments");
      const payData = await payRes.json();
      setPayments(payData);

      // جلب الإعدادات الحالية
      const setRes = await fetch("http://127.0.0.1:8005/api/settings");
  ...
  const handleSaveSettings = async () => {
    ...
    await fetch("http://127.0.0.1:8005/api/admin/settings", {
      method: "POST",
  ...
  const handleApprove = async (paymentId: string) => {
    ...
    await fetch(`http://127.0.0.1:8005/api/admin/payments/approve/${paymentId}`, { method: "POST" });
    fetchData();
  };
